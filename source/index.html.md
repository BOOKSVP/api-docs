...
| `booking[tickets]` | `true`   | List of product objects; each with `ticket_id` (required), and optionally `size`. Find `ticket_id` through the event object. `ticket_id` is a UUID. See [The event object](#the-event-object) for more details. |
